...
import (
	...
	"github.com/garyleungsky/provider-learn/apis"
	learn "github.com/garyleungsky/provider-learn/internal/controller"
	"github.com/garyleungsky/provider-learn/internal/version"
)

func main() {
	var (
		app            = kingpin.New(filepath.Base(os.Args[0]), "Learn support for Crossplane.").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-learn"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-learn",
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add Learn APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := controller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-learn:%s", version.Version))),
		}
		o.ChangeLogOptions = &clo
	}

	kingpin.FatalIfError(customresourcesgate.Setup(mgr, o), "Cannot setup CRD gate controller")
	kingpin.FatalIfError(learn.SetupGated(mgr, o), "Cannot setup Learn controllers")
